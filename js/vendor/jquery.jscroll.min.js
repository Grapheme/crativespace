        ...
        function _load() {
            ...
            return _checkNextHref(data) && $e.animate({scrollTop: $inner.outerHeight()}, 0, function() {
                $inner.find('div.jscroll-added').last().load(data.nextHref, function(r, status, xhr) {
                    ...
                    if (_options.autoTrigger) {
                        _nextWrap($next);
						$(".jscroll-added a.advanced").each(function(i,element){$(element).off('click').on('click',function(event){mt.ExtendText(element,event)})});
						$(".jscroll-added a.сollapse").each(function(i,element){$(element).off('click').on('click',function(event){mt.CollapseText(element,event)})});
					} else {
